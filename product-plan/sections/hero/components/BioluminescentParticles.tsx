...
export function BioluminescentParticles({
  ...
  // Generate random particles - fewer, simpler
  const particles = useMemo<Particle[]>(() => {
    return Array.from({ length: count }, (_, i) => ({
      ...
      duration: Math.random() * 6 + 8, // 8-14s (slower = less CPU)
      delay: Math.random() * 8,
    }))
  }, [count])
  ...
  return (
    ...
    >
      ...
      {/* Just 3 larger accent particles */}
      {[0, 1, 2].map((i) => (
        <div
          key={`large-${i}`}
      ...
      <style>{`
        ...
        .animate-large-float {
          animation: large-float ease-in-out infinite;
        }
      `}</style>
    </div>
  )
